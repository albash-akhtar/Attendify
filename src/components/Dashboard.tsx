...
// 🔑 Aapka Asli Corrected Supabase Connection URL aur API Key
  ...
  try {
    ...
    if (data) {
      cacheSet('c_rec', data.map((r: any) => ({
        ...
        // 🔑 Agar login aur logout time Supabase mein same hain ya checkout null hai, toh frontend usey ongoing handle karega
...
export async function updateAttendanceRecord(id: string, updates: Partial<AttendanceRecord>) {
  ...
  try {
    ...
    // 🔑 MAIN FIX: Agar edit ke dauran checkout khali chora hai, toh Supabase mein explicit null jayega taake shift live chalti rahe
    if (updates.checkOut !== undefined) {
      d.logout_time = updates.checkOut === "" ? null : updates.checkOut;
    }
    if (updates.totalHours !== undefined) {
      d.total_hours = updates.totalHours;
    }
